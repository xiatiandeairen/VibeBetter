...
import { toPercent, getBenchmarkLevel } from '@vibebetter/shared';
          ...
          {overviewLoading ? (
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <div>
                ...
                />
                {overview?.aiSuccessRate != null && (
                  <BenchmarkBadge level={getBenchmarkLevel('aiSuccessRate', overview.aiSuccessRate)} />
                )}
              </div>
              <div>
                <MetricCard
                ...
                />
                {overview?.aiStableRate != null && (
                  <BenchmarkBadge level={getBenchmarkLevel('aiStableRate', overview.aiStableRate)} />
                )}
              </div>
              <div>
                <MetricCard
                ...
                />
                {overview?.psriScore != null && (
                  <BenchmarkBadge level={getBenchmarkLevel('psriScore', overview.psriScore)} />
                )}
              </div>
              <div>
                <MetricCard
                ...
                />
                {overview?.tdiScore != null && (
                  <BenchmarkBadge level={getBenchmarkLevel('tdiScore', overview.tdiScore)} />
                )}
              </div>
            </div>
          )}
...
function BenchmarkBadge({ level }: { level: string }) {
  return (
    <div className="mt-1 flex justify-start px-4">
      <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${benchmarkColorMap[level] ?? 'bg-zinc-800 text-zinc-400'}`}>
        {level}
      </span>
    </div>
  );
